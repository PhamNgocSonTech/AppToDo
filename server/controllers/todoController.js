...
const createToDo = async (req, res) => {
  try {
    ...
    const newTask = await newTodo.save();
...
const getAllToDo = async(req, res) => {
  try {
    ...
    if(req.user._id !== currentUser) return res.status(500).json("You can't get to do other account")
    // if(currentToDo ) return res.status(400).json("You Not Create To Do")
    const todos = await ToDo.find({userId: currentUser});
